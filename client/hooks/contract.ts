...
// ============================================================
// Cross Chain Bridge — Contract Methods (Manual ScVal)
// ============================================================

/**
 * Lock funds for cross-chain transfer.
 * Calls: lock_funds(user: Address, amount: i128, target_chain: Symbol, target_address: BytesN<32>)
 */
export async function lockFunds(
  ...
  return callContract(
    ...
    [
      ...
      toScValI128(amount),
      toScValSymbol(targetChain),
      toScValBytes(targetAddress),
    ],
    caller,
    true
  );
...
/**
 * Release funds after cross-chain verification.
 * Calls: release_funds(user: Address, amount: i128, proof_hash: BytesN<32>)
 */
...
/**
 * Get locked funds for a user (read-only).
 * Calls: get_locked(user: Address) -> (i128, Symbol, BytesN<32>)
 * Returns: [amount: bigint, targetChain: string, targetAddress: string]
 */
export async function getLocked(user: string) {
  ...
  if (result && Array.isArray(result)) {
    return {
      amount: BigInt(result[0]?.value ?? 0),
      targetChain: String(result[1] ?? ""),
      targetAddress: String(result[2] ?? ""),
    };
  }
  return null;
}

// Helper for BytesN<32>
function toScValBytes(hex: string): xdr.ScVal {
  const bytes = hex.replace(/^0x/, "");
  const u8 = new Uint8Array(32);
  for (let i = 0; i < 32; i++) {
    u8[i] = parseInt(bytes.substr(i * 2, 2), 16) || 0;
  }
  return nativeToScVal(Buffer.from(u8), { type: "bytes" });
}

// Helper for Symbol
function toScValSymbol(value: string): xdr.ScVal {
